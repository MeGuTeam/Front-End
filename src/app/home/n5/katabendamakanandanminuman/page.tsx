...
import { getNounFoodAndDrink } from "@/features/home/services/data"
import { trackerNounFoodAndDrink } from "@/features/home/services/tracker"
...
interface NounFoodAndDrinkData {
    id: number;
    romaji?: string;
    reading?: string;
    word: string;
...
const NounFoodAndDrinkPage = () => {
    const [nounFoodAndDrink, setNounFoodAndDrink] = useState<NounFoodAndDrinkData[]>([]);
    ...
    useEffect(() => {
        const fetchNounFoodAndDrink = async () => {
            try {
                const data = await getNounFoodAndDrink();
                setNounFoodAndDrink(data || []);
            } catch (e) {
        ...
        fetchNounFoodAndDrink();
    }, []);

    const handleNounFoodAndDrinkClick = async (colorId: number, currentStatus: boolean) => {
        if (updating === colorId) return;

        try {            
            setUpdating(colorId);
            const newStatus = !currentStatus;
            await trackerNounFoodAndDrink(colorId, newStatus);

            setNounFoodAndDrink(prev => prev.map(data =>
    ...
    const completedCount = nounFoodAndDrink.filter(p => p.status).length;
    const progressPercentage = nounFoodAndDrink.length > 0 ? (completedCount / nounFoodAndDrink.length) * 100 : 0;
    ...
    return (
        <div className="bg-black min-h-screen text-white pt-18">
                ...
                >
                    <div className="flex items-center gap-3 mb-3">
                        <h1 className="text-3xl font-bold">
                            Kata Makanan dan Minuman N5
                        </h1>
                    </div>
                    <p>Pelajari Kosakata Makanan dan Minuman untuk menguasai bahasa Jepang</p>
                </motion.div>
            </section>
                ...
                >
                  ...
                  <div className="flex gap-2 items-center mb-4">
                    ...
                    <div className="flex gap-1 text-sm">
                        ...
                        <span className="text-white/50">&gt;</span>
                        <span className="text-purple-400 font-medium">Kata Makanan dan Minuman N5</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="bg-purple-500/20 px-3 py-1 rounded-full">
                        <span className="text-purple-500 font-semibold text-sm">
                            {completedCount}/{nounFoodAndDrink.length}
                        </span>
                    </div>
                  </div>
            ...
            <section className="container px-4 mx-auto mt-8 pb-8">
                ...
                {!loading && !error && nounFoodAndDrink.length > 0 && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        {nounFoodAndDrink.map((data, index) => (
                            <motion.div
                                key={data.id}
                            ...
                            >
                                <div
                                    ...
                                    onClick={() => handleNounFoodAndDrinkClick(data.id, data.status)}
                                >
                ...
                {!loading && !error && nounFoodAndDrink.length === 0 && (
                    <div className="text-center py-20">
                        ...
                        <p className="text-white/60 text-lg">Belum ada materi kata Makanan dan Minuman N5 tersedia</p>
                    </div>
                )}
            </section>
        </div>
...
export default NounFoodAndDrinkPage