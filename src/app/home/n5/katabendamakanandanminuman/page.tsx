...
import { NounFoodandDrinkN5 } from '@/features/home/types/materin5';

const NounFoodAndDrinkPage = () => {
  const [nounFoodAndDrink, setNounFoodAndDrink] = useState<
    NounFoodandDrinkN5[]
  >([]);
  ...
  const fetchNounFoodAndDrink = async (
    ...
    try {
      setLoading(true);
      const data = await getNounFoodAndDrink(setUpdatedData);
  ...
  useEffect(() => {
    if (updatedData === false) {
      fetchNounFoodAndDrink(setUpdatedData);
      setLoading(false);
    }
  }, [updatedData]);

  const handleNounFoodAndDrinkClick = async (
    foodDrinkId: number,
    currentStatus: boolean
  ) => {
    try {
      setUpdating(foodDrinkId);
      await trackerNounFoodAndDrink(foodDrinkId, currentStatus, setUpdatedData);
    } catch (e) {
      console.error('Failed to update noun food and drink status: ', e);
  ...
  const progressPercentage =
    nounFoodAndDrink.length > 0
      ? (completedCount / nounFoodAndDrink.length) * 100
      : 0;
      ...
      <section className="container mx-auto px-4 pt-8">
        ...
        >
          <div className="flex items-center gap-3 mb-3">
            <h1 className="text-3xl font-bold">Kata Makanan dan Minuman N5</h1>
          </div>
          <p>
            Pelajari Kosakata Makanan dan Minuman untuk menguasai bahasa Jepang
          </p>
        </motion.div>
      </section>
            ...
            {nounFoodAndDrink.map((data, index) => (
              ...
              >
                <div
                  ...
                  onClick={() =>
                    handleNounFoodAndDrinkClick(data.id, data.status)
                  }
                >
        ...
        {!loading && !error && nounFoodAndDrink.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-white/60 text-lg">
              Belum ada materi kata Makanan dan Minuman N5 tersedia
            </p>
          </div>
        )}
      </section>
