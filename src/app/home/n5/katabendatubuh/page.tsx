...
import { NounBodyN5 } from '@/features/home/types/materin5';

const NounBodyPage = () => {
  const [nounBody, setNounBody] = useState<NounBodyN5[]>([]);
  ...
  const fetchNounBody = async (
    ...
    try {
      setLoading(true);
      const data = await getNounBody(setUpdatedData);
      setNounBody(data || []);
    } catch (e) {
  ...
  useEffect(() => {
    if (updatedData === false) {
      fetchNounBody(setUpdatedData);
      setLoading(false);
    }
  }, [updatedData]);

  const handleNounBodyClick = async (
    bodyId: number,
    currentStatus: boolean
  ) => {
    try {
      setUpdating(bodyId);
      await trackerNounBody(bodyId, currentStatus, setUpdatedData);
    } catch (e) {
  ...
  const progressPercentage =
    nounBody.length > 0 ? (completedCount / nounBody.length) * 100 : 0;
      ...
      <section className="container mx-auto px-4 pt-8">
        ...
        >
          <div className="flex items-center gap-3 mb-3">
            <h1 className="text-3xl font-bold">Tubuh Manusia N5</h1>
          </div>
          <p>
            Pelajari kosakata untuk mendeskripsikan tubuh manusia dalam bahasa
            jepang
          </p>
        </motion.div>
      </section>
        ...
        {!loading && !error && nounBody.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-white/60 text-lg">
              Belum ada materi kata tubuh N5 tersedia
            </p>
          </div>
        )}
      </section>
