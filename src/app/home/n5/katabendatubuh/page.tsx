...
import { getNounBody } from "@/features/home/services/data"
import { trackerNounBody } from "@/features/home/services/tracker"
...
const NounBodyPage = () => {
    const [nounBody, setNounBody] = useState<NounBodyData[]>([]);
    ...
    useEffect(() => {
        const fetchNounBody = async () => {
            try {
                const data = await getNounBody();
                setNounBody(data || []);
            } catch (e) {
    ...
    const handleNounBodyClick = async (bodyId: number, currentStatus: boolean) => {
        if (updating === bodyId) return;

        try {
            console.log("bodyId data: ", bodyId);
            
            setUpdating(bodyId);
            console.log("updating state: ", updating);
            const newStatus = !currentStatus;
            await trackerNounBody(bodyId, newStatus);

            setNounBody(prev => prev.map(data =>
                data.id === bodyId ? { ...data, status:newStatus} : data
            ));
        } catch (e) {
    ...
    const completedCount = nounBody.filter(p => p.status).length;
    const progressPercentage = nounBody.length > 0 ? (completedCount / nounBody.length) * 100 : 0;
        ...
        <div className="bg-black min-h-screen text-white pt-18">
            ...
            <section className="container mx-auto px-4 pt-8">
                ...
                >
                    ...
                    <p>Pelajari kosakata untuk mendeskripsikan tubuh manusia dalam bahasa jepang</p>
                </motion.div>
            </section>
                ...
                >
                  ...
                  <div className="flex gap-2 items-center mb-4">
                    ...
                    <div className="flex gap-1 text-sm">
                        ...
                        <span className="text-white/50">&gt;</span>
                        <span className="text-purple-400 font-medium">Kata Tubuh Manusia N5</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="bg-purple-500/20 px-3 py-1 rounded-full">
                        <span className="text-purple-500 font-semibold text-sm">
                            {completedCount}/{nounBody.length}
                        </span>
                    </div>
                  </div>
            ...
            <section className="container px-4 mx-auto mt-8 pb-8">
                ...
                {!loading && !error && nounBody.length > 0 && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        {nounBody.map((data, index) => (
                            <motion.div
                                key={data.id}
                            ...
                            >
                                <div
                                    ...
                                    onClick={() => handleNounBodyClick(data.id, data.status)}
                                >
                ...
                {!loading && !error && nounBody.length === 0 && (
                    <div className="text-center py-20">
                        ...
                        <p className="text-white/60 text-lg">Belum ada materi kata tubuh N5 tersedia</p>
                    </div>
                )}
            </section>
        </div>
