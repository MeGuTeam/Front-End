...
import { NounAuxNumberN5 } from '@/features/home/types/materin5';
import { AiOutlineLoading } from 'react-icons/ai';

const NounAuxNumberPage = () => {
  const [nounAuxNumbers, setNounAuxNumbers] = useState<NounAuxNumberN5[]>([]);
  ...
  const fetchNounAuxNumbers = async (
    ...
    try {
      setLoading(true);
      const data = await getNounAuxNumber(setUpdatedData);
  ...
  useEffect(() => {
    if (updatedData === false) {
      fetchNounAuxNumbers(setUpdatedData);
      setLoading(false);
    }
  }, [updatedData]);

  const handleNounAuxNumberClick = async (
    auxNumberId: number,
    currentStatus: boolean
  ) => {
    try {
      setUpdating(auxNumberId);
      await trackerNounAuxNumber(auxNumberId, currentStatus, setUpdatedData);
    } catch (e) {
  ...
  const progressPercentage =
    nounAuxNumbers.length > 0
      ? (completedCount / nounAuxNumbers.length) * 100
      : 0;
      ...
      <section className="container mx-auto px-4 pt-8">
        ...
        >
          <div className="flex items-center gap-3 mb-3">
            <h1 className="text-3xl font-bold">Kata Bantu Bilangan N5</h1>
          </div>
          <p className="text-white/70 text-lg">
            Pelajari kata bantu bilangan level N5 untuk menguasai bahasa Jepang
          </p>
        </motion.div>
      </section>
        ...
        {!loading && !error && nounAuxNumbers.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-white/60 text-lg">
              Belum ada materi kata bantu bilangan N5 tersedia
            </p>
          </div>
        )}
      </section>
