...
import { getNounAuxNumber } from '@/features/home/services/data'
import { trackerNounAuxNumber } from '@/features/home/services/tracker'
...
const NounAuxNumberPage = () => {
    const [nounAuxNumbers, setNounAuxNumbers] = useState<NounAuxNumberData[]>([]);
    ...
    useEffect(() => {
        const fetchNounAuxNumbers = async () => {
          try {
            const data = await getNounAuxNumber();
            setNounAuxNumbers(data || []);
          } catch (e) {
    ...
    const handleNounAuxNumberClick = async (auxNumberId: number, currentStatus: boolean) => {
      if (updating === auxNumberId) return; // Prevent double clicks
      
      try {
        setUpdating(auxNumberId);
        const newStatus = !currentStatus;
        await trackerNounAuxNumber(auxNumberId, newStatus);

        setNounAuxNumbers(prev => prev.map(data =>
          data.id === auxNumberId ? { ...data, status: newStatus} : data
        ));
      } catch (e) {
        console.error("Failed to update noun aux number status: ", e);
    ...
    const completedCount = nounAuxNumbers.filter(p => p.status).length;
    const progressPercentage = nounAuxNumbers.length > 0 ? (completedCount / nounAuxNumbers.length) * 100 : 0;
    ...
    return (
        <div className='bg-black min-h-screen text-white pt-18'>
            ...
            >
              <div className="flex items-center gap-3 mb-3">
                <h1 className="text-3xl font-bold">
                  Kata Bantu Bilangan N5
                </h1>
              </div>
              <p className="text-white/70 text-lg">Pelajari kata bantu bilangan level N5 untuk menguasai bahasa Jepang</p>
            </motion.div>
          </section>
            ...
            >
              ...
              <div className='flex gap-2 items-center mb-4'>
                ...
                <div className='flex gap-1 text-sm'>
                  ...
                  <span className="text-white/50">&gt;</span> 
                  <span className='text-purple-400 font-medium'>Kata Bantu Bilangan N5</span>
                </div>
              </div>
              ...
              <div className='flex items-center justify-between mb-3'>
                ...
                <div className="bg-purple-500/20 px-3 py-1 rounded-full">
                  <span className="text-purple-500 font-semibold text-sm">
                    {completedCount}/{nounAuxNumbers.length}
                  </span>
                </div>
              </div>
          ...
          <section className='container px-4 mx-auto mt-8 pb-8'>
            ...
            {!loading && !error && nounAuxNumbers.length > 0 && (
              <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4'>
                {nounAuxNumbers.map((data, index) => (
                  <motion.div 
                    key={data.id} 
                  ...
                  >
                    <div 
                      ...
                      onClick={() => handleNounAuxNumberClick(data.id, data.status)}
                    >
                      <div className="p-6 text-center">
                        {/* Noun Aux Number Word */}
                        <div className='mb-6'>
            ...
            {!loading && !error && nounAuxNumbers.length === 0 && (
              <div className="text-center py-20">
                ...
                <p className="text-white/60 text-lg">Belum ada materi kata bantu bilangan N5 tersedia</p>
              </div>
            )}
          </section>
        </div>
...
export default NounAuxNumberPage