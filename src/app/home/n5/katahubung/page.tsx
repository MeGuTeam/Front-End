...
const KataHubungPages = () => {
    ...
    useEffect(() => {
        const fetchConjunctions = async () => {
          try {
            const data = await getConjunctionN5();
    ...
    const handleConjunctionClick = async (conjunctionId: number, currentStatus: boolean) => {
      if (updating === conjunctionId) return; // Prevent double clicks
      
      try {
        setUpdating(conjunctionId);
        const newStatus = !currentStatus;
        await trackerConjunction(conjunctionId, newStatus);

        setConjunctions(prev => prev.map(data =>
          data.id === conjunctionId ? { ...data, status: newStatus} : data
        ));
      } catch (e) {
    ...
    const progressPercentage = conjunctions.length > 0 ? (completedCount / conjunctions.length) * 100 : 0;
        ...
        <div className='bg-black min-h-screen text-white pt-18'>
          ...
          <section className="container mx-auto px-4 pt-8">
            ...
            >
              ...
              <p className="text-white/70 text-lg">Pelajari kata hubung level N5 untuk menguasai bahasa Jepang</p>
            </motion.div>
          </section>
          ...
          <section className='container px-4 mx-auto mt-8 pb-8'>
            ...
            {!loading && !error && conjunctions.length === 0 && (
              <div className="text-center py-20">
                ...
                <p className="text-white/60 text-lg">Belum ada materi kata hubung N5 tersedia</p>
              </div>
            )}
          </section>
        </div>
