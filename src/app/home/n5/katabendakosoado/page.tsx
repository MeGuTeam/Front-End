...
import { getNounKosoado } from "@/features/home/services/data"
import { trackerNounKosoado } from "@/features/home/services/tracker"
...
const NounHomeAppliancesPage = () => {
    const [nounKosoado, setNounKosoado] = useState<NounKosoadoData[]>([]);
    ...
    useEffect(() => {
        const fetchNounKosoado = async () => {
            try {
                const data = await getNounKosoado();
                setNounKosoado(data || []);
            } catch (e) {
    ...
    const handleNounKosoadoClick = async (colorId: number, currentStatus: boolean) => {
        if (updating === colorId) return;

        try {            
            setUpdating(colorId);
            const newStatus = !currentStatus;
            await trackerNounKosoado(colorId, newStatus);

            setNounKosoado(prev => prev.map(data =>
    ...
    const progressPercentage = nounKosoado.length > 0 ? (completedCount / nounKosoado.length) * 100 : 0;
        ...
        <div className="bg-black min-h-screen text-white pt-18">
            ...
            <section className="container mx-auto px-4 pt-8">
                ...
                >
                    ...
                    <p>Pelajari Kosakata Kosoado untuk menguasai bahasa Jepang</p>
                </motion.div>
            </section>
                ...
                >
                  ...
                  <div className="flex gap-2 items-center mb-4">
                    ...
                    <div className="flex gap-1 text-sm">
                        ...
                        <span className="text-white/50">&gt;</span>
                        <span className="text-purple-400 font-medium">Kata Kosoado N5</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="bg-purple-500/20 px-3 py-1 rounded-full">
                        <span className="text-purple-500 font-semibold text-sm">
                            {completedCount}/{nounKosoado.length}
                        </span>
                    </div>
                  </div>
            ...
            <section className="container px-4 mx-auto mt-8 pb-8">
                ...
                {!loading && !error && nounKosoado.length > 0 && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        {nounKosoado.map((data, index) => (
                            <motion.div
                                key={data.id}
                            ...
                            >
                                <div
                                    ...
                                    onClick={() => handleNounKosoadoClick(data.id, data.status)}
                                >
