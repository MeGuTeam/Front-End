...
import { NounPositionN5 } from '@/features/home/types/materin5';

const NounPositionPage = () => {
  const [nounPosition, setNounPosition] = useState<NounPositionN5[]>([]);
  ...
  const fetchNounPosition = async (
    ...
    try {
      setLoading(true);
      const data = await getNounPosition(setUpdatedData);
  ...
  useEffect(() => {
    if (updatedData === false) {
      fetchNounPosition(setUpdatedData);
      setLoading(false);
    }
  }, [updatedData]);

  const handleNounPositionClick = async (
    positionId: number,
    currentStatus: boolean
  ) => {
    try {
      setUpdating(positionId);
      await trackerNounPosition(positionId, currentStatus, setUpdatedData);
    } catch (e) {
      console.error('Failed to update noun position status: ', e);
  ...
  const progressPercentage =
    nounPosition.length > 0 ? (completedCount / nounPosition.length) * 100 : 0;
      ...
      <section className="container mx-auto px-4 pt-8">
        ...
        >
          <div className="flex items-center gap-3 mb-3">
            <h1 className="text-3xl font-bold">Kata Posisi N5</h1>
          </div>
        ...
        >
          ...
          <div className="flex gap-2 items-center mb-4">
            ...
            <div className="flex gap-1 text-sm">
              ...
              <span className="text-white/50">&gt;</span>
              <span className="text-blue-400 font-medium">Kata Posisi N5</span>
            </div>
          </div>
        ...
        {!loading && !error && nounPosition.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-white/60 text-lg">
              Belum ada materi kata posisi N5 tersedia
            </p>
          </div>
        )}
      </section>
