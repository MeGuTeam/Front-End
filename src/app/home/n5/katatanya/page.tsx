...
const KataTanyaPages = () => {
  const [questionWords, setQuestionWords] = useState<KataTanyaN5[]>([]);
  ...
  const fetchQuestionWords = async (
    ...
    try {
      setLoading(true);
      const data = await getQuestionWord(setUpdatedData);
  ...
  useEffect(() => {
    if (updatedData === false) {
      fetchQuestionWords(setUpdatedData);
      setLoading(false);
    }
  }, [updatedData]);

  const handleQuestionWordClick = async (
    questionWordId: number,
    currentStatus: boolean
  ) => {
    try {
      setUpdating(questionWordId);
      await trackerQuestionWord(questionWordId, currentStatus, setUpdatedData);
    } catch (e) {
  ...
  const progressPercentage =
    questionWords.length > 0
      ? (completedCount / questionWords.length) * 100
      : 0;
      ...
      <section className="container mx-auto px-4 pt-8">
        ...
        >
          <div className="flex items-center gap-3 mb-3">
            <h1 className="text-3xl font-bold">Kata Tanya N5</h1>
          </div>
          <p className="text-white/70 text-lg">
            Pelajari kata tanya level N5 untuk menguasai bahasa Jepang
          </p>
        </motion.div>
      </section>
        ...
        {!loading && !error && questionWords.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-white/60 text-lg">
              Belum ada materi kata tanya N5 tersedia
            </p>
          </div>
        )}
      </section>
