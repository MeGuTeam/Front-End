...
import { getQuestionWord } from '@/features/home/services/data'
import { trackerQuestionWord } from '@/features/home/services/tracker'
...
const KataTanyaPages = () => {
    const [questionWords, setQuestionWords] = useState<QuestionWordData[]>([]);
    ...
    useEffect(() => {
        const fetchQuestionWords = async () => {
          try {
            const data = await getQuestionWord();
    ...
    const handleQuestionWordClick = async (questionWordId: number, currentStatus: boolean) => {      
      try {
        setUpdating(questionWordId);
        const newStatus = !currentStatus;
        await trackerQuestionWord(questionWordId, newStatus);

        setQuestionWords(prev => prev.map(data =>
          data.id === questionWordId ? { ...data, status: newStatus} : data
        ));
      } catch (e) {
        console.error("Failed to update question word status: ", e);
    ...
    const completedCount = questionWords.filter(p => p.status).length;
    const progressPercentage = questionWords.length > 0 ? (completedCount / questionWords.length) * 100 : 0;
        ...
        <div className='bg-black min-h-screen text-white pt-18'>
          ...
          <section className="container mx-auto px-4 pt-8">
            ...
            >
              ...
              <p className="text-white/70 text-lg">Pelajari kata tanya level N5 untuk menguasai bahasa Jepang</p>
            </motion.div>
          </section>
            ...
            >
              ...
              <div className='flex gap-2 items-center mb-4'>
                ...
                <div className='flex gap-1 text-sm'>
                  ...
                  <span className="text-white/50">&gt;</span> 
                  <span className='text-blue-400 font-medium'>Kata Tanya N5</span>
                </div>
              </div>
              ...
              <div className='flex items-center justify-between mb-3'>
                ...
                <div className="bg-blue-500/20 px-3 py-1 rounded-full">
                  <span className="text-blue-500 font-semibold text-sm">
                    {completedCount}/{questionWords.length}
                  </span>
                </div>
              </div>
          ...
          <section className='container px-4 mx-auto mt-8 pb-8'>
            ...
            {!loading && !error && questionWords.length > 0 && (
              <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4'>
                {questionWords.map((data, index) => (
                  <motion.div 
                    key={data.id} 
                  ...
                  >
                    <div 
                      ...
                      onClick={() => handleQuestionWordClick(data.id, data.status)}
                    >
            ...
            {!loading && !error && questionWords.length === 0 && (
              <div className="text-center py-20">
                ...
                <p className="text-white/60 text-lg">Belum ada materi kata tanya N5 tersedia</p>
              </div>
            )}
          </section>
        </div>
