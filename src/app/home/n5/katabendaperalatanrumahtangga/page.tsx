...
import { NounHomeAppliancesN5 } from '@/features/home/types/materin5';

const NounHomeAppliancesPage = () => {
  const [nounHomeAppliances, setNounHomeAppliances] = useState<
    NounHomeAppliancesN5[]
  >([]);
  ...
  const fetchNounHomeAppliances = async (
    ...
    try {
      setLoading(true);
      const data = await getNounHomeAppliances(setUpdatedData);
  ...
  useEffect(() => {
    if (updatedData === false) {
      fetchNounHomeAppliances(setUpdatedData);
      setLoading(false);
    }
  }, [updatedData]);

  const handleNounHomeAppliancesClick = async (
    applianceId: number,
    currentStatus: boolean
  ) => {
    try {
      setUpdating(applianceId);
      await trackerNounHomeAppliances(
        applianceId,
        currentStatus,
        setUpdatedData
      );
    } catch (e) {
      console.error('Failed to update noun home appliances status: ', e);
  ...
  const progressPercentage =
    nounHomeAppliances.length > 0
      ? (completedCount / nounHomeAppliances.length) * 100
      : 0;
    ...
    <div className="bg-black min-h-screen text-white pt-18">
      ...
      <section className="container mx-auto px-4 pt-8">
        ...
        >
          ...
          <p>
            Pelajari Kosakata Peralatan Rumah Tangga untuk menguasai bahasa
            Jepang
          </p>
        </motion.div>
      </section>
            ...
            {nounHomeAppliances.map((data, index) => (
              ...
              >
                <div
                  ...
                  onClick={() =>
                    handleNounHomeAppliancesClick(data.id, data.status)
                  }
                >
        ...
        {!loading && !error && nounHomeAppliances.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-white/60 text-lg">
              Belum ada materi kata Peralatan rumah tangga N5 tersedia
            </p>
          </div>
        )}
      </section>
