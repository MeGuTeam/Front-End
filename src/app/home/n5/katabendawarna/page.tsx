...
import { NounColorN5 } from '@/features/home/types/materin5';

const NounColorPage = () => {
  const [nounColor, setNounColor] = useState<NounColorN5[]>([]);
  ...
  const fetchNounColor = async (
    ...
    try {
      setLoading(true);
      const data = await getNounColor(setUpdatedData);
      setNounColor(data || []);
    } catch (e) {
  ...
  useEffect(() => {
    if (updatedData === false) {
      fetchNounColor(setUpdatedData);
      setLoading(false);
    }
  }, [updatedData]);

  const handleNounColorClick = async (
    colorId: number,
    currentStatus: boolean
  ) => {
    try {
      setUpdating(colorId);
      await trackerNounColor(colorId, currentStatus, setUpdatedData);
    } catch (e) {
      console.error('Failed to update noun color status: ', e);
  ...
  const progressPercentage =
    nounColor.length > 0 ? (completedCount / nounColor.length) * 100 : 0;
      ...
      <section className="container mx-auto px-4 pt-8">
        ...
        >
          <div className="flex items-center gap-3 mb-3">
            <h1 className="text-3xl font-bold">Warna N5</h1>
          </div>
        ...
        >
          ...
          <div className="flex gap-2 items-center mb-4">
            ...
            <div className="flex gap-1 text-sm">
              ...
              <span className="text-white/50">&gt;</span>
              <span className="text-blue-400 font-medium">Kata Warna N5</span>
            </div>
          </div>
        ...
        {!loading && !error && nounColor.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-white/60 text-lg">
              Belum ada materi kata kota N5 tersedia
            </p>
          </div>
        )}
      </section>
