...
import { NounAnimalandPlantN5 } from '@/features/home/types/materin5';
import { AiOutlineLoading } from 'react-icons/ai';

const NounAnimalAndPlantPages = () => {
  const [nounAnimalAndPlants, setNounAnimalAndPlants] = useState<
    NounAnimalandPlantN5[]
  >([]);
  ...
  const fetchNounAnimalAndPlants = async (
    ...
    try {
      setLoading(true);
      const data = await getNounAnimalAndPlant(setUpdatedData);
  ...
  useEffect(() => {
    if (updatedData === false) {
      fetchNounAnimalAndPlants(setUpdatedData);
      setLoading(false);
    }
  }, [updatedData]);

  const handleNounAnimalAndPlantClick = async (
    animalAndPlantId: number,
    currentStatus: boolean
  ) => {
    try {
      setUpdating(animalAndPlantId);
      await trackerNounAnimalAndPlant(
        animalAndPlantId,
        currentStatus,
        setUpdatedData
      );
  ...
  const progressPercentage =
    nounAnimalAndPlants.length > 0
      ? (completedCount / nounAnimalAndPlants.length) * 100
      : 0;
      ...
      <section className="container mx-auto px-4 pt-8">
        ...
        >
          ...
          <p className="text-white/70 text-lg">
            Pelajari kata benda hewan dan tumbuhan level N5 untuk menguasai
            bahasa Jepang
          </p>
        </motion.div>
      </section>
            ...
            {nounAnimalAndPlants.map((data, index) => (
              ...
              >
                <div
                  ...
                  onClick={() =>
                    handleNounAnimalAndPlantClick(data.id, data.status)
                  }
                >
        ...
        {!loading && !error && nounAnimalAndPlants.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-white/60 text-lg">
              Belum ada materi kata benda hewan dan tumbuhan N5 tersedia
            </p>
          </div>
        )}
      </section>
