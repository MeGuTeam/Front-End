...
import { getNounAnimalAndPlant } from '@/features/home/services/data'
import { trackerNounAnimalAndPlant } from '@/features/home/services/tracker'
...
interface NounAnimalAndPlantData {
  id: number;
  word: string;
  romaji?: string;
  reading?: string;
...
const NounAnimalAndPlantPages = () => {
    const [nounAnimalAndPlants, setNounAnimalAndPlants] = useState<NounAnimalAndPlantData[]>([]);
    ...
    useEffect(() => {
        const fetchNounAnimalAndPlants = async () => {
          try {
            const data = await getNounAnimalAndPlant();
            setNounAnimalAndPlants(data || []);
          } catch (e) {
        ...
        fetchNounAnimalAndPlants();
    }, [])

    const handleNounAnimalAndPlantClick = async (animalAndPlantId: number, currentStatus: boolean) => {
      if (updating === animalAndPlantId) return; // Prevent double clicks
      
      try {
        setUpdating(animalAndPlantId);
        const newStatus = !currentStatus;
        await trackerNounAnimalAndPlant(animalAndPlantId, newStatus);

        setNounAnimalAndPlants(prev => prev.map(data =>
          data.id === animalAndPlantId ? { ...data, status: newStatus} : data
        ));
      } catch (e) {
        console.error("Failed to update noun animal and plant status: ", e);
    ...
    const completedCount = nounAnimalAndPlants.filter(p => p.status).length;
    const progressPercentage = nounAnimalAndPlants.length > 0 ? (completedCount / nounAnimalAndPlants.length) * 100 : 0;
    ...
    return (
        <div className='bg-black min-h-screen text-white pt-18'>
            ...
            >
              <div className="flex items-center gap-3 mb-3">
                <h1 className="text-3xl font-bold">
                  Kata Benda Hewan & Tumbuhan N5
                </h1>
              </div>
              <p className="text-white/70 text-lg">Pelajari kata benda hewan dan tumbuhan level N5 untuk menguasai bahasa Jepang</p>
            </motion.div>
          </section>
            ...
            >
              ...
              <div className='flex gap-2 items-center mb-4'>
                ...
                <div className='flex gap-1 text-sm'>
                  ...
                  <span className="text-white/50">&gt;</span> 
                  <span className='text-purple-400 font-medium'>Kata Benda Hewan & Tumbuhan N5</span>
                </div>
              </div>
              ...
              <div className='flex items-center justify-between mb-3'>
                ...
                <div className="bg-purple-500/20 px-3 py-1 rounded-full">
                  <span className="text-purple-500 font-semibold text-sm">
                    {completedCount}/{nounAnimalAndPlants.length}
                  </span>
                </div>
              </div>
          ...
          {/* Noun Animal And Plant List Section */}
          <section className='container px-4 mx-auto mt-8 pb-8'>
            ...
            {!loading && !error && nounAnimalAndPlants.length > 0 && (
              <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4'>
                {nounAnimalAndPlants.map((data, index) => (
                  <motion.div 
                    key={data.id} 
                  ...
                  >
                    <div 
                      ...
                      onClick={() => handleNounAnimalAndPlantClick(data.id, data.status)}
                    >
                      <div className="p-6 text-center">
                        {/* Noun Animal And Plant Word */}
                        <div className='mb-6'>
            ...
            {!loading && !error && nounAnimalAndPlants.length === 0 && (
              <div className="text-center py-20">
                ...
                <p className="text-white/60 text-lg">Belum ada materi kata benda hewan dan tumbuhan N5 tersedia</p>
              </div>
            )}
          </section>
        </div>
...
export default NounAnimalAndPlantPages