...
import { NounActivityN5 } from '@/features/home/types/materin5';
import { AiOutlineLoading } from 'react-icons/ai';

const NounActivityPages = () => {
  const [nounActivities, setNounActivities] = useState<NounActivityN5[]>([]);
  ...
  const fetchNounActivities = async (
    ...
    try {
      setLoading(true);
      const data = await getNounActivityN5(setUpdatedData);
  ...
  useEffect(() => {
    if (updatedData === false) {
      fetchNounActivities(setUpdatedData);
      setLoading(false);
    }
  }, [updatedData]);

  const handleNounActivityClick = async (
    nounActivityId: number,
    currentStatus: boolean
  ) => {
    try {
      setUpdating(nounActivityId);
      await trackerNounActivity(nounActivityId, currentStatus, setUpdatedData);
    } catch (e) {
  ...
  const progressPercentage =
    nounActivities.length > 0
      ? (completedCount / nounActivities.length) * 100
      : 0;
      ...
      <section className="container mx-auto px-4 pt-8">
        ...
        >
          <div className="flex items-center gap-3 mb-3">
            <h1 className="text-3xl font-bold">Kata Benda Aktivitas N5</h1>
          </div>
          <p className="text-white/70 text-lg">
            Pelajari kata benda aktivitas level N5 untuk menguasai bahasa Jepang
          </p>
        </motion.div>
      </section>
        ...
        {!loading && !error && nounActivities.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-white/60 text-lg">
              Belum ada materi kata benda aktivitas N5 tersedia
            </p>
          </div>
        )}
      </section>
