...
import { getNounActivityN5 } from '@/features/home/services/data'
import { trackerNounActivity } from '@/features/home/services/tracker'
...
const NounActivityPages = () => {
    const [nounActivities, setNounActivities] = useState<NounActivityData[]>([]);
    ...
    useEffect(() => {
        const fetchNounActivities = async () => {
          try {
            const data = await getNounActivityN5();
            setNounActivities(data || []);
          } catch (e) {
    ...
    const handleNounActivityClick = async (nounActivityId: number, currentStatus: boolean) => {      
      try {
        setUpdating(nounActivityId);
        const newStatus = !currentStatus;
        await trackerNounActivity(nounActivityId, newStatus);

        setNounActivities(prev => prev.map(data =>
          data.id === nounActivityId ? { ...data, status: newStatus} : data
        ));
      } catch (e) {
        console.error("Failed to update noun activity status: ", e);
    ...
    const completedCount = nounActivities.filter(p => p.status).length;
    const progressPercentage = nounActivities.length > 0 ? (completedCount / nounActivities.length) * 100 : 0;
    ...
    return (
        <div className='bg-black min-h-screen text-white pt-18'>
            ...
            >
              <div className="flex items-center gap-3 mb-3">
                <h1 className="text-3xl font-bold">
                  Kata Benda Aktivitas N5
                </h1>
              </div>
              <p className="text-white/70 text-lg">Pelajari kata benda aktivitas level N5 untuk menguasai bahasa Jepang</p>
            </motion.div>
          </section>
            ...
            >
              ...
              <div className='flex gap-2 items-center mb-4'>
                ...
                <div className='flex gap-1 text-sm'>
                  <div className="hover:text-green-400 cursor-pointer transition-colors">
                  ...
                  <span className="text-white/50">&gt;</span> 
                  <span className='text-green-400 font-medium'>Kata Benda Aktivitas N5</span>
                </div>
              </div>
              ...
              <div className='flex items-center justify-between mb-3'>
                <div className='flex items-center gap-3'>
                  <IoBookOutline className="w-5 h-5 text-green-400" />
                  <p className="text-white/80 font-medium">Progress Belajar Anda</p>
                </div>
                <div className="bg-green-500/20 px-3 py-1 rounded-full">
                  <span className="text-green-500 font-semibold text-sm">
                    {completedCount}/{nounActivities.length}
                  </span>
                </div>
              </div>
          ...
          <section className='container px-4 mx-auto mt-8 pb-8'>
            ...
            {!loading && !error && nounActivities.length > 0 && (
              <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4'>
                {nounActivities.map((data, index) => (
                  <motion.div 
                    key={data.id} 
                  ...
                  >
                    <div 
                      ...
                      onClick={() => handleNounActivityClick(data.id, data.status)}
                    >
            ...
            {!loading && !error && nounActivities.length === 0 && (
              <div className="text-center py-20">
                ...
                <p className="text-white/60 text-lg">Belum ada materi kata benda aktivitas N5 tersedia</p>
              </div>
            )}
          </section>
        </div>
...
export default NounActivityPages