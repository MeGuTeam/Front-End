...
import { NounCityN5 } from '@/features/home/types/materin5';

const NounCityPage = () => {
  const [nounCity, setNounCity] = useState<NounCityN5[]>([]);
  ...
  const fetchNounCity = async (
    ...
    try {
      setLoading(true);
      const data = await getNounCity(setUpdatedData);
      setNounCity(data || []);
    } catch (e) {
  ...
  useEffect(() => {
    if (updatedData === false) {
      fetchNounCity(setUpdatedData);
      setLoading(false);
    }
  }, [updatedData]);

  const handleNounCityClick = async (
    cityId: number,
    currentStatus: boolean
  ) => {
    try {
      setUpdating(cityId);
      await trackerNounCity(cityId, currentStatus, setUpdatedData);
    } catch (e) {
      console.error('Failed to update noun city status: ', e);
  ...
  const progressPercentage =
    nounCity.length > 0 ? (completedCount / nounCity.length) * 100 : 0;
      ...
      <section className="container mx-auto px-4 pt-8">
        ...
        >
          <div className="flex items-center gap-3 mb-3">
            <h1 className="text-3xl font-bold">Kota N5</h1>
          </div>
        ...
        >
          ...
          <div className="flex gap-2 items-center mb-4">
            ...
            <div className="flex gap-1 text-sm">
              ...
              <span className="text-white/50">&gt;</span>
              <span className="text-blue-400 font-medium">Kata Kota N5</span>
            </div>
          </div>
