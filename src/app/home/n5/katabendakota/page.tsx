...
import { getNounCity } from "@/features/home/services/data"
import { trackerNounCity } from "@/features/home/services/tracker"
...
const NounCityPage = () => {
    const [nounCity, setNounCity] = useState<NounCityData[]>([]);
    ...
    useEffect(() => {
        const fetchNounCity = async () => {
            try {
                const data = await getNounCity();
                setNounCity(data || []);
            } catch (e) {
    ...
    const handleNounCityClick = async (cityId: number, currentStatus: boolean) => {
        if (updating === cityId) return;

        try {            
            setUpdating(cityId);
            const newStatus = !currentStatus;
            await trackerNounCity(cityId, newStatus);

            setNounCity(prev => prev.map(data =>
                data.id === cityId ? { ...data, status:newStatus} : data
            ));
        } catch (e) {
    ...
    const completedCount = nounCity.filter(p => p.status).length;
    const progressPercentage = nounCity.length > 0 ? (completedCount / nounCity.length) * 100 : 0;
        ...
        <div className="bg-black min-h-screen text-white pt-18">
            ...
            <section className="container mx-auto px-4 pt-8">
                ...
                >
                    ...
                    <p>Pelajari Kosakata Kota untuk menguasai bahasa Jepang</p>
                </motion.div>
            </section>
                ...
                >
                  ...
                  <div className="flex gap-2 items-center mb-4">
                    ...
                    <div className="flex gap-1 text-sm">
                        ...
                        <span className="text-white/50">&gt;</span>
                        <span className="text-purple-400 font-medium">Kata Kota N5</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="bg-purple-500/20 px-3 py-1 rounded-full">
                        <span className="text-purple-500 font-semibold text-sm">
                            {completedCount}/{nounCity.length}
                        </span>
                    </div>
                  </div>
            ...
            <section className="container px-4 mx-auto mt-8 pb-8">
                ...
                {!loading && !error && nounCity.length > 0 && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        {nounCity.map((data, index) => (
                            <motion.div
                                key={data.id}
                            ...
                            >
                                <div
                                    ...
                                    onClick={() => handleNounCityClick(data.id, data.status)}
                                >
