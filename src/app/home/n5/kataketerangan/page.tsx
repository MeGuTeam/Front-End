...
import { getAdverbN5 } from '@/features/home/services/data'
import { trackerAdverb } from '@/features/home/services/tracker'
...
const KataKeteranganPages = () => {
    const [adverb, setAdverb] = useState<any[]>([]);
    const [error, setError] = useState("");
    const [loading, setLoading] = useState(true);
    const router = useRouter();

    useEffect(() => {
        const fetchAdverb = async () => {
          try {
            const data = await getAdverbN5();
            setAdverb(data);
          } catch (e) {
    ...
    const handleAdverbClick = async (adverbId: number, currentStatus: boolean) => {
      try {
        const newStatus = !currentStatus;
        await trackerAdverb(adverbId, newStatus);

        setAdverb(prev => prev.map(data =>
          data.id === adverbId ? { ...data, status: newStatus} : data
        ));
      } catch (e) {
        console.error("Failed to update adverb status: ", e);
      }
    }

    const completedCount = adverb.filter(p => p.status).length;
    const progressPercentage = adverb.length > 0 ? (completedCount / adverb.length) * 100 : 0;
        ...
        <div className='bg-black min-h-svh text-white pt-18'>
          ...
          <section className="container mx-auto px-4 pt-8">
            ...
            >
              ...
              <p className="text-white/70 text-lg">Pelajari kata keterangan level N5 untuk menguasai bahasa Jepang</p>
            </motion.div>
          </section>
            ...
            >
              ...
              <div className='flex gap-2 items-center mb-4'>
                ...
                <div className='flex gap-1 text-sm'>
                  ...
                  <span className="text-white/50">&gt;</span> 
                  <span className='text-blue-400 font-medium hover:cursor-pointer'>Kata Keterangan N5</span>
                </div>
              </div>
          ...
          <section className='container px-4 mx-auto mt-8 pb-8'>
                ...
                {adverb.map((data, index) => (
                  ...
                  >
                    <div 
                      ...
                      onClick={() => handleAdverbClick(data.id, data.status)}
                    >
            ...
            {/* Empty State */}
            {!loading && !error && adverb.length === 0 && (
              <div className="text-center py-20">
                <HiOutlineSparkles className="w-16 h-16 text-white/30 mx-auto mb-4" />
                <p className="text-white/60 text-lg">Belum ada materi kata keterangan N5 tersedia</p>
              </div>
            )}
          </section>
        </div>
...
export default KataKeteranganPages