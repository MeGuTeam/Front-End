...
import { KataKeteranganN5 } from '@/features/home/types/materin5';
import { AiOutlineLoading } from 'react-icons/ai';

const KataKeteranganPages = () => {
  const [adverb, setAdverb] = useState<KataKeteranganN5[]>([]);
  ...
  const fetchAdverb = async (
    ...
    try {
      setLoading(true);
      const data = await getAdverbN5(setUpdatedData);
      setAdverb(data);
    } catch (e) {
  ...
  useEffect(() => {
    if (updatedData === false) {
      fetchAdverb(setUpdatedData);
      setLoading(false);
    }
  }, [updatedData]);

  const handleAdverbClick = async (
    adverbId: number,
    currentStatus: boolean
  ) => {
    try {
      setUpdating(adverbId);
      await trackerAdverb(adverbId, currentStatus, setUpdatedData);
    } catch (e) {
  ...
  const progressPercentage =
    adverb.length > 0 ? (completedCount / adverb.length) * 100 : 0;
  ...
  return (
    <div className="bg-black min-h-svh text-white pt-18">
      ...
      <section className="container mx-auto px-4 pt-8">
        ...
        >
          <h1 className="text-3xl font-bold mb-2">Kata Keterangan N5</h1>
          <p className="text-white/70 text-lg">
            Pelajari kata keterangan level N5 untuk menguasai bahasa Jepang
          </p>
        </motion.div>
      </section>
        ...
        {!loading && !error && adverb.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-white/60 text-lg">
              Belum ada materi kata keterangan N5 tersedia
            </p>
          </div>
        )}
      </section>
