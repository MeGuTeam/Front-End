...
import { getNounSchool } from "@/features/home/services/data"
import { trackerNounSchool } from "@/features/home/services/tracker"
...
const NounSchoolPage = () => {
    const [nounSchool, setNounSchool] = useState<NounSchoolData[]>([]);
    ...
    useEffect(() => {
        const fetchNounSchool = async () => {
            try {
                const data = await getNounSchool();
                setNounSchool(data || []);
            } catch (e) {
    ...
    const handleNounSchoolClick = async (colorId: number, currentStatus: boolean) => {
        if (updating === colorId) return;

        try {            
            setUpdating(colorId);
            const newStatus = !currentStatus;
            await trackerNounSchool(colorId, newStatus);

            setNounSchool(prev => prev.map(data =>
    ...
    const progressPercentage = nounSchool.length > 0 ? (completedCount / nounSchool.length) * 100 : 0;
        ...
        <div className="bg-black min-h-screen text-white pt-18">
            ...
            <section className="container mx-auto px-4 pt-8">
                ...
                >
                    ...
                    <p>Pelajari Kosakata Sekolah untuk menguasai bahasa Jepang</p>
                </motion.div>
            </section>
                ...
                >
                  ...
                  <div className="flex gap-2 items-center mb-4">
                    ...
                    <div className="flex gap-1 text-sm">
                        ...
                        <span className="text-white/50">&gt;</span>
                        <span className="text-purple-400 font-medium">Kata Sekolah N5</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="bg-purple-500/20 px-3 py-1 rounded-full">
                        <span className="text-purple-500 font-semibold text-sm">
                            {completedCount}/{nounSchool.length}
                        </span>
                    </div>
                  </div>
            ...
            <section className="container px-4 mx-auto mt-8 pb-8">
                ...
                {!loading && !error && nounSchool.length > 0 && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        {nounSchool.map((data, index) => (
                            <motion.div
                                key={data.id}
                            ...
                            >
                                <div
                                    ...
                                    onClick={() => handleNounSchoolClick(data.id, data.status)}
                                >
                ...
                {!loading && !error && nounSchool.length === 0 && (
                    <div className="text-center py-20">
                        ...
                        <p className="text-white/60 text-lg">Belum ada materi kata Sekolah N5 tersedia</p>
                    </div>
                )}
            </section>
        </div>
