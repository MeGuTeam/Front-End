...
import { getAdjectiveN5 } from '@/features/home/services/data'
import { trackerAdjectiveN5 } from '@/features/home/services/tracker'
...
const KataSifatPages = () => {
    const [adjective, setAdjective] = useState<any[]>([]);
    ...
    useEffect(() => {
        const fetchAdjective = async () => {
          try {
            const data = await getAdjectiveN5();
            setAdjective(data);
          } catch (e) {
    ...
    const handleAdjectiveN5Click = async (adjectiveId: number, currentStatus: boolean) => {
      try {
        const newStatus = !currentStatus;
        await trackerAdjectiveN5(adjectiveId, newStatus);

        setAdjective(prev => prev.map(data =>
          data.id === adjectiveId ? { ...data, status: newStatus} : data
        ));
      } catch (e) {
        console.error("Failed to update adjective status: ", e);
      }
    }

    const completedCount = adjective.filter(p => p.status).length;
    const progressPercentage = adjective.length > 0 ? (completedCount / adjective.length) * 100 : 0;
        ...
        <div className='bg-black min-h-svh text-white pt-18'>
          ...
          <section className="container mx-auto px-4 pt-8">
            ...
            >
              ...
              <p className="text-white/70 text-lg">Pelajari Kata Sifat level N5 untuk menguasai bahasa Jepang</p>
            </motion.div>
          </section>
            ...
            >
              ...
              <div className='flex gap-2 items-center mb-4'>
                ...
                <div className='flex gap-1 text-sm'>
                  ...
                  <span className="text-white/50">&gt;</span> 
                  <span className='text-blue-400 font-medium hover:cursor-pointer'>Kata Sifat N5</span>
                </div>
              </div>
              ...
              <div className='flex items-center justify-between mb-3'>
                ...
                <div className="bg-blue-500/20 px-3 py-1 rounded-full">
                  <span className="text-blue-500 font-semibold text-sm">
                    {completedCount}/{adjective.length}
                  </span>
                </div>
              </div>
          ...
          <section className='container px-4 mx-auto mt-8 pb-8'>
                ...
                {adjective.map((data, index) => (
                  ...
                  >
                    <div 
                      ...
                      onClick={() => handleAdjectiveN5Click(data.id, data.status)}
                    >
                        ...
                        <div className='grid grid-cols-1 gap-3 mb-4'>
                          ...
                          {data.type && (
                            <div className='bg-white/5 rounded-lg p-3'>
                              ...
                              <p className="text-sm font-semibold text-white">{data.type}</p>
                            </div>
                          )}
                        </div>
            ...
            {/* Empty State */}
            {!loading && !error && adjective.length === 0 && (
              <div className="text-center py-20">
                <TbLetterCase className="w-16 h-16 text-white/30 mx-auto mb-4" />
                <p className="text-white/60 text-lg">Belum ada materi adjective N5 tersedia</p>
              </div>
            )}
          </section>
        </div>
