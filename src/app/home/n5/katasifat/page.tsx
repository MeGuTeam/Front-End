...
import { KataSifatN5 } from '@/features/home/types/materin5';
import { AiOutlineLoading } from 'react-icons/ai';

const KataSifatPages = () => {
  const [adjective, setAdjective] = useState<KataSifatN5[]>([]);
  ...
  const fetchAdjective = async (
    ...
    try {
      setLoading(true);
      const data = await getAdjectiveN5(setUpdatedData);
      setAdjective(data);
    } catch (e) {
  ...
  useEffect(() => {
    if (updatedData === false) {
      fetchAdjective(setUpdatedData);
      setLoading(false);
    }
  }, [updatedData]);

  const handleAdjectiveN5Click = async (
    adjectiveId: number,
    currentStatus: boolean
  ) => {
    try {
      setUpdating(adjectiveId);
      await trackerAdjectiveN5(adjectiveId, currentStatus, setUpdatedData);
    } catch (e) {
  ...
  const progressPercentage =
    adjective.length > 0 ? (completedCount / adjective.length) * 100 : 0;
  ...
  return (
    <div className="bg-black min-h-svh text-white pt-18">
      ...
      <section className="container mx-auto px-4 pt-8">
        ...
        >
          <h1 className="text-3xl font-bold mb-2">Kata Sifat N5</h1>
          <p className="text-white/70 text-lg">
            Pelajari Kata Sifat level N5 untuk menguasai bahasa Jepang
          </p>
        </motion.div>
      </section>
        ...
        {!loading && !error && adjective.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-white/60 text-lg">
              Belum ada materi adjective N5 tersedia
            </p>
          </div>
        )}
      </section>
