...
import { KataKerjaN5 } from '@/features/home/types/materin5';
import { AiOutlineLoading } from 'react-icons/ai';

const KataKerjaPages = () => {
  const [verbs, setVerbs] = useState<KataKerjaN5[]>([]);
  ...
  const fetchVerbs = async (
    ...
    try {
      setLoading(true);
      const data = await getVerbN5(setUpdatedData);
      setVerbs(data || []);
    } catch (e) {
  ...
  const handleVerbClick = async (verbId: number, currentStatus: boolean) => {
    try {
      setUpdating(verbId);
      await trackerVerb(verbId, currentStatus, setUpdatedData);
    } catch (e) {
  ...
  const progressPercentage =
    verbs.length > 0 ? (completedCount / verbs.length) * 100 : 0;
      ...
      <section className="container mx-auto px-4 pt-8">
        ...
        >
          <div className="flex items-center gap-3 mb-3">
            <h1 className="text-3xl font-bold">Kata Kerja N5</h1>
          </div>
          <p className="text-white/70 text-lg">
            Pelajari kata kerja level N5 untuk menguasai bahasa Jepang
          </p>
        </motion.div>
      </section>
        ...
        {!loading && !error && verbs.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-white/60 text-lg">
              Belum ada materi kata kerja N5 tersedia
            </p>
          </div>
        )}
      </section>
