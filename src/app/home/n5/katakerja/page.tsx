...
import { getVerbN5 } from '@/features/home/services/data'
import { trackerVerb } from '@/features/home/services/tracker'
...
const KataKerjaPages = () => {
    ...
    const handleVerbClick = async (verbId: number, currentStatus: boolean) => {
      if (updating === verbId) return; // Prevent double clicks
      
      try {
        setUpdating(verbId);
        const newStatus = !currentStatus;
        await trackerVerb(verbId, newStatus);

        setVerbs(prev => prev.map(data =>
          data.id === verbId ? { ...data, status: newStatus} : data
        ));
      } catch (e) {
        console.error("Failed to update verb status: ", e);
    ...
    const completedCount = verbs.filter(p => p.status).length;
    const progressPercentage = verbs.length > 0 ? (completedCount / verbs.length) * 100 : 0;
        ...
        <div className='bg-black min-h-screen text-white pt-18'>
          ...
          <section className="container mx-auto px-4 pt-8">
            ...
            >
              ...
              <p className="text-white/70 text-lg">Pelajari kata kerja level N5 untuk menguasai bahasa Jepang</p>
            </motion.div>
          </section>
            ...
            >
              ...
              <div className='flex gap-2 items-center mb-4'>
                ...
                <div className='flex gap-1 text-sm'>
                  ...
                  <span className="text-white/50">&gt;</span> 
                  <span className='text-blue-400 font-medium'>Kata Kerja N5</span>
                </div>
              </div>
          ...
          <section className='container px-4 mx-auto mt-8 pb-8'>
            ...
            {!loading && !error && verbs.length > 0 && (
              <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4'>
                {verbs.map((data, index) => (
                  <motion.div 
                    key={data.id} 
                  ...
                  >
                    <div 
                      ...
                      onClick={() => handleVerbClick(data.id, data.status)}
                    >
