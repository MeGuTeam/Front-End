...
const KatakanaPages = () => {
  ...
  const handleKatakanaClick = async (
    ...
    try {
      ...
      await trackerKatakana(katakanaId, currentStatus, setUpdatedData);
    } catch (error) {
      console.error('Failed to update katakana status:', error);
    } finally {
      setUpdating(null);
    }
  };
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="bg-blue-500/20 px-3 py-1 rounded-full">
              <span className="text-blue-500 font-semibold text-sm">
                {completedCount}/{katakana.length}
              </span>
            </div>
          </div>
