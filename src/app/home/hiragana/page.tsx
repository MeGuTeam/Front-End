...
const HiraganaPages = () => {
  ...
  const fetchHiragana = async (setUpdatedData: React.Dispatch<React.SetStateAction<boolean>>) => {
    try {
      setLoading(true);
  ...
  const handleHiraganaClick = async (hiraganaId: number, currentStatus: boolean) => {
    try {
  ...
  const progressPercentage = hiragana.length > 0 ? (completedCount / hiragana.length) * 100 : 0;
    ...
    <div className="bg-black min-h-svh text-white pt-18">
      ...
      <section className="container mx-auto px-4 pt-8">
        ...
        >
          ...
          <p className="text-white/70 text-lg">Pelajari sistem penulisan hiragana untuk menguasai bahasa Jepang</p>
        </motion.div>
      </section>
        ...
        >
          ...
          <div className='flex gap-2 items-center mb-4'>
            ...
            <div className='flex gap-1 text-sm'>
              ...
              <span className='text-blue-400 font-medium hover:cursor-pointer'>Hiragana</span>
            </div>
          </div>
      ...
      <section className='container px-4 mx-auto mt-8 pb-8'>
        ...
        {!loading && !error && hiragana.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-white/60 text-lg">Belum ada materi hiragana tersedia</p>
          </div>
        )}
      </section>
    </div>
