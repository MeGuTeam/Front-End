...
const ParticlesPage = () => {
  ...
  const fetchParticles = async (setUpdatedData: React.Dispatch<React.SetStateAction<boolean>>) => {
    try {
      setLoading(true);
  ...
  const handleParticleClick = async (particleId: number, currentStatus: boolean) => {
    try {
  ...
  const progressPercentage = particles.length > 0 ? (completedCount / particles.length) * 100 : 0;
        ...
        >
          ...
          <div className='flex gap-2 items-center mb-4'>
            ...
            <div className='flex gap-1 text-sm'>
              ...
              <span className='text-blue-400 font-medium hover:cursor-pointer'>Particle</span>
            </div>
          </div>
